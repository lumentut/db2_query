...
class ConnectionPoolTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @pool = DB2Query::ConnectionPool.new DB2Query::Base.connection_pool.spec
  end
  ...
  def test_anonymous_class_exception
    anonymous = Class.new(DB2Query::Base)
  ...
  class ConnectionTestModel < DB2Query::Base
  end
  ...
  def test_connection_notification_is_called
    ...
    ConnectionTestModel.establish_connection :dqunit

    assert_equal [:config, :connection_id, :spec_name], payloads[0].keys.sort
    assert_equal "ConnectionPoolTest::ConnectionTestModel", payloads[0][:spec_name]
  ensure
  ...
  def test_non_bang_disconnect_and_clear_reloadable_connections_throw_exception_if_threads_dont_return_their_conns
    ...
    [:disconnect, :clear_reloadable_connections].each do |group_action_method|
      pool.with_connection do |connection|
        assert_raises(ActiveRecord::ExclusiveConnectionTimeoutError) do
